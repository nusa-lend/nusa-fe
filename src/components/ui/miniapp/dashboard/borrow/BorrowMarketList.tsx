...
interface BorrowMarketListProps {
  onMarketSelect: (market: BorrowingMarket) => void;
  markets: BorrowingMarket[];
  isLoading: boolean;
  error: any;
}

export default function BorrowMarketList({ onMarketSelect, markets, isLoading, error }: BorrowMarketListProps) {
  const [searchTerm, setSearchTerm] = useState('');
  const { data: marketBalances, isLoading: balancesLoading } = useAggregatedBalances(markets);

  const handleMarketClick = (market: BorrowingMarket) => {
    ...
    <div className="w-full px-4">
      ...
      <div className="px-2 py-3 rounded-3xl border border-white/25 ">
        ...
        <div className="flex justify-between items-center mt-4">
          <Tooltip
            content="Use your RWA tokens as collateral to borrow stablecoin. Collateral does not earn interest."
            trigger="click"
            position="right"
            className="z-50"
          >
            ...
            filteredMarkets.map(market => (
              <div key={market.id} onClick={() => handleMarketClick(market)} className="cursor-pointer">
                <ItemCard
                  ...
                  apyColor="#279E73"
                  imageSize={36}
                  flagSrc={market.token.logoCountry}
